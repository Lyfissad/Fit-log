...
export default function Auth() {
  ...
  return userLoading ? (
    <div className="flex items-center justify-center h-full w-full">
      <AiOutlineLoading3Quarters className="fill-text-color size-15 animate-spin" />
    </div>
  ) : (
    <div className="bg-grayBlack h-screen animate-fadeIn w-full pt-5 text-text-pri">
      <Image
      ...
      <div className="flex items-center justify-center h-8/12">
        <Link href={"/auth/login"}>
          <button className="py-4 px-2 rounded-lg cursor-pointer bg-priAccent">
            Login/SignUp
          </button>
        </Link>
      </div>
