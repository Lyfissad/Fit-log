  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const res = await fetch(
        "https://fitlog-back-1.onrender.com/auth/signup",
        {
          method: "POST",
          headers: {
      ...
      >
        ...
        <div className="flex">
          ...
          {showIcon ? (
            <BiSolidShow
              onClick={switchShow}
              className="absolute right-10 fill-text-color cursor-pointer size-5 mt-2"
            />
          ) : (
            <BiSolidHide
              onClick={switchShow}
        ...
        <button
          type="submit"
          className="w-[20rem] h-[3rem] cursor-pointer rounded-lg bg-priAccent mx-auto"
        >
          {buttonState}
        </button>
      </form>
